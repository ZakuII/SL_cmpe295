...
function pqEditInitQuestion(){
	...
	$.getJSON("/kohana/question/SocialQuest/getQuestion",function(data){
		...
		var pqLength = data.SocialQuestion.length;
		var pqLenCount = 0;
		var pqIDsave="";
		
		sessionStorage.pqJSON = JSON.stringify(data.SocialQuestion);
		
		$.each(data.SocialQuestion, function(i,user){
...
$(document).delegate('#pqEditSave', 'click', function() {
	$.ajax({
		type: "POST",
		url: "/kohana/question/SocialQuest/postShow",
