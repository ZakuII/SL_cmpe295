...
function pqEditInitQuestion(){
	...
	$.getJSON("/kohana/question/EnvironmentalQuest/getQuestion",function(data){
		...
		var pqLength = data.EnvironmentalQuestion.length;
		var pqLenCount = 0;
		var pqIDsave="";
		
		sessionStorage.pqJSON = JSON.stringify(data.EnvironmentalQuestion);
		
		$.each(data.EnvironmentalQuestion, function(i,user){
...
$(document).delegate('#pqEditSave', 'click', function() {
	$.ajax({
		type: "POST",
		url: "/kohana/question/EnvironmentalQuest/postShow",
