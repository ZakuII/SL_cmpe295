//$("#PQedit").live('pagebeforeshow', function(){
//$(document).live('pagebeforecreate', '#PQedit', function(){
...
function pqEditInitQuestion(){
	...
	$.getJSON("/kohana/question/personQuest/getQuestion",function(data){
		...
		var pqLength = data.personalQuestion.length;
		var pqLenCount = 0;
		var pqIDsave="";
		
		sessionStorage.pqJSON = JSON.stringify(data.personalQuestion);
		
		$.each(data.personalQuestion, function(i,user){
...
$(document).delegate('#pqEditTest', 'click', function() {
	...
	var gettest = sessionStorage.pqJSON;
	var gettest2 = JSON.parse(sessionStorage.pqJSON);
	//console.log("Testing ",gettest2[0].question)
	$.each(gettest2, function(i,user){
		console.log(user.id)
	});
...
$(document).delegate('#pqEditSave', 'click', function() {
	...
	$.ajax({
		type: "POST",
		url: "/kohana/question/personQuest/postShow",
