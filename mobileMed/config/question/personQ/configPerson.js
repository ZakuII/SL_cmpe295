...
$(document).bind('pageinit', '#PersonQuestion',function(){
	...
	/*$("#pqBtnPreview").click(function(){
		//$("#pqPrevAuth").html("clicked");
		var PrevBtnOutStr = "";
		var radioResult = $('input[name=perQ-radio-choice]:checked').val()
		...
		if(prevResult.prevBtnChkOutCnt<1){
			var passed = "Question: " + textResult + " Type:" + radioResult + " NumField" + menuResultInt + " ArgField" + prevResult.perQuestArgStr;
			...
			var passed2 = "Question: " + sessionStorage.perQQuestion + " Type:" + sessionStorage.perQType + " NumField" + sessionStorage.perQNumField + " ArgField" + sessionStorage.perQArgField;
			$("#abc1").append(passed2);
			$.mobile.changePage("#PersonQuestionPreview");
		}
		else{
...
/*
$(document).delegate('#pqBtnCancel', 'click', function() {
		//$.mobile.changePage("#PersonQuestionPreview");
		
		//$.mobile.changePage( "#PersonQuestionPreview", {
	$.mobile.changePage("newQuestion.html#PersonQuestionPreview", {
		transition: "pop",
		//reverse: false,
		//changeHash: false,
		role: "dialog"
...
$(document).delegate('#pqBtnConfirm', 'click', function() {
	//alert("got pressed");
		...
		$.ajax({
		type: "POST",
		url: "/kohana/config/personQuest/post",
		data: { 
		...
		//$.mobile.changePage("#PersonQuestion");
		//$.mobile.changePage("#PQedit");
...
function PrevRenderQuestion(prevQuestObj){
	...
	if(prevQuestObj.type=="text"){
		//var pQRenArgSplit = prevQuestObj.arg.split("|");
		//var pQRenArgSplit2;	
		...
		for(pqRenCnt=0; pqRenCnt<prevQuestObj.numField; pqRenCnt++){
			...
			pQRenArgSplit3 = pQRenArgSplit2[2].split(",");
				...
				//prevRenOut+= "["+ pqRenCnt2 + "]" +pQRenArgSplit3[pqRenCnt2];
		...
		for(pqRenCnt=0; pqRenCnt<prevQuestObj.numField; pqRenCnt++){
			...
			prevRenOut+= '<select name="menufields'+ pqRenCnt +'" id="menufields'+ pqRenCnt +'" data-native-menu="true">';
				...
				for(pqRenCnt2=0;pqRenCnt2 < pQRenArgSplit3.length;pqRenCnt2++){
				//for(pqRenCnt2=0;pqRenCnt2 < 2;pqRenCnt2++){
...
function expand_personQ(radioResult, menuResultInt){
	...
	//var testing = "<div data-role=\"fieldcontain\"><label for=\"name\">Text Input:</label><input type=\"text\" name=\"name\" id=\"name\" value=\"\"  /></div>";
	...
	if(menuResultInt != 0){
		...
		//pOutStr = pOutStr + "<p id=\"abc1\">Text Area ABC</p>";
		$("#perQ-more").html(pOutStr);
