...
function pqEditInitQuestion(){
	...
	$.getJSON("/kohana/question/FamilyQuest/getQuestion",function(data){
		...
		var pqLength = data.FamilyQuestion.length;
		var pqLenCount = 0;
		var pqIDsave="";
		
		sessionStorage.pqJSON = JSON.stringify(data.FamilyQuestion);
		
		$.each(data.FamilyQuestion, function(i,user){
...
$(document).delegate('#pqEditSave', 'click', function() {
	$.ajax({
		type: "POST",
		url: "/kohana/question/FamilyQuest/postShow",
