...
function loadSwipe(window, Util, PhotoSwipe){
		//console.log("pageshow!!");
		var options = {
		getToolbar: function(){
			return '<div class="ps-toolbar-close" style="padding-top: 12px;">Close</div><div class="ps-toolbar-previous" style="padding-top: 12px;">Previous</div><div class="ps-toolbar-next" style="padding-top: 12px;">Next</div>';
		}},
...
function getImages(){
	...
	$('body').addClass('ui-loading');
	var url = "/kohana/config/procedure/getImages";
	$.getJSON(url, {ppatID:sessionStorage.ppatId}, function(data){	
		var imgField = '';
		//console.log("length : " +data.length);
		imgField = '<ul class="gallery">';
		if(data != null){
		if(data.length > 0){
			$.each(data, function(i,file){
				//console.log("file :"+i+" , "+ file);
				imgField += '<li><figure><a href="/kohana/userdata/images/'+sessionStorage.ppatId+'/'+ file +'" rel="external"><img src="/kohana/userdata/images/'+sessionStorage.ppatId+'/'+ file +'"  width="120" height="120" alt="Image 001" /></a><figcaption style="text-align:center;">'+file+'</figcaption></figure><li>';
				//imgField += '<li><a href="/kohana/userdata/images/'+sessionStorage.ppatId+'/'+ file +'" rel="external"><img src="/kohana/userdata/images/'+sessionStorage.ppatId+'/file.jpg" alt="'+file+'" /></a></li>';
			});	
			imgField += '</ul>';
			$("#gallery").html(imgField); 
			$("#gallery").trigger("create");
			loadSwipe(window, window.jQuery, window.Code.PhotoSwipe);
		}
		else{
			imgField = '<h3><em>No Images Found</em></h3>';
			$("#gallery").html(imgField); 
			$("#gallery").trigger("create");			
		}
		}
...
function getProcResult(){
	...
	.error(function() { $("#procTable").html("No Result Found"); resultFlag = 0;})
...
function renderProcTable2(proc){
	...
	if(resultFlag == 1){
		...
		for(var i = 0; i < proc.numArg; i++){
			ParamParse = argsParse[i].split(":");		
			procTable += '<th>'+ParamParse[0]+'</br>';		//parameter name
			procTable += ParamParse[1]+'</br>';				//parameter units
		
			argTemp[i%modNum] = ParamParse[0];				//store parameter in an temp array
		
			procGraphSel += '<option value="'+proc.name+'|'+ParamParse[0]+'|'+ParamParse[2]+'|'+ParamParse[3]+'">'+ParamParse[0]+'</option>';
		
			if(ParamParse[2] == '-'){
				procTable += '< '+ParamParse[3]+'</th>';
				min[i%modNum] = 0;
				max[i%modNum] = ParamParse[3];
			}
			else if(ParamParse[3] == '+'){
				procTable += '> '+ParamParse[2]+'</th>';
				min[i%modNum] = ParamParse[2];
				max[i%modNum] = 0;
			}
			else{
				procTable += ParamParse[2]+' - '+ParamParse[3]+'</th>';
				min[i%modNum] = ParamParse[2];
				max[i%modNum] = ParamParse[3];
			}
		
			if((i)%modNum == (modNum-1) && (i+1) < proc.numArg){		//if reach mod amount
			
				for(var k = 0; k < results.length; k++){
					procTable += '</tr>';
					tempTime = results[k]['time'].split(" ");
					//procTable += '<tr><td>'+results[k]['time']+'</td>';
					procTable += '<tr><td>'+tempTime[0]+'</td>';
					for(var j = 0; j < modNum; j++){
						cellNum = results[k][argTemp[j]];
						if(max[j] == 0){
							if(cellNum < min[j]){
								procTable += '<td style="background-color:lightgreen;">'+cellNum+'</td>';
							}
							else{
								procTable += '<td>'+cellNum+'</td>';
							}
						...
						//procTable += '<td>'+results[k][argTemp[j]]+'</td>';
					}			
				}			
				procTable += '</tr></table>';
				procTable += '<table border="1" width="100%"><tr>';	
			...
			else if((i+1) == proc.numArg){							//print line if not w/in mod
		...
		$("#procHead").html(procGraphSel);
		$("#procTable").html(procTable);
		//$("#procTable").html(procGraphSel+procTable);
		$("#procHead").trigger("create");
		$("#procTable").trigger("create");
		showPtable();
		}//if has property
...
$(document).delegate('#procImgAll', 'click', function() {	
	showAllGallery();
});
