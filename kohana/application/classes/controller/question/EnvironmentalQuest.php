...
//question
class Controller_question_EnvironmentalQuest extends Controller {
	...
	//update new question ANSWER
	 public function action_post() {
	 	...
	 	$personA = new Model_EnvironmentalAnswer();
	...
	public function action_postShow(){
		...
		for ($i=0; $i<sizeof($parsedpqid); $i++){
			...
			$personA = new Model_EnvironmentalQuestion($parsedpqid[$i]);
	...
	public function action_getAnswer(){
    	...
    	if(isset($_GET['id'])){
    		...
    		$personQ = ORM::factory('EnvironmentalQuestion')->find_all(); 
    		$personA = new Model_EnvironmentalAnswer($_GET['id']);
	    	$tblSize = sizeof($personQ);
    		$tblCnt = 0;
    	
    	
    	echo'{ "EnvironmentalAnswer": [';
    	//echo "</br>";
    	foreach ($personQ as $pQ){
    		//if($pQ->display==1){
    			echo '{ "question":"';
