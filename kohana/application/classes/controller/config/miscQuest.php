...
//config
class Controller_config_MiscQuest extends Controller {
	...
	public function action_index(){
	 ...
	 public function action_post() {
        $personQ = new Model_MiscQuestion();
		...
		$raw_sql_here = "alter table MiscAnswer add column `".$sid."` text";
	...
	public function action_edit() {
		...
		if(isset($_POST['edit']) && isset($_POST['id'])){
			...
			$personQ = new Model_MiscQuestion($_POST['id']);
			...
			if($personQ->arrangement == -1){
				...
				if($_POST['edit']=="remove"){
					...
					$raw_sql_here = "alter table MiscAnswer drop column `".$sid."`";
					$query = DB::query(NULL, $raw_sql_here);
					$result = $query->execute();
					$personQ->delete();
