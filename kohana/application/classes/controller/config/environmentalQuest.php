...
//config
class Controller_config_EnvironmentalQuest extends Controller {

	public function action_index(){
	
        $personQ = ORM::factory('EnvironmentalQuestion')->find_all(); 

		echo 'hello';
		echo "</br>";
		
		$columns = Database::instance()->list_columns('EnvironmentalQuestion');
	 ...
	 public function action_post() {
        $personQ = new Model_EnvironmentalQuestion();
		...
		$raw_sql_here = "alter table EnvironmentalAnswer add column `".$sid."` text";
	...
	public function action_edit() {
		...
		if(isset($_POST['edit']) && isset($_POST['id'])){
			...
			$personQ = new Model_EnvironmentalQuestion($_POST['id']);
			...
			if($personQ->arrangement == -1){
				...
				if($_POST['edit']=="remove"){
					...
					$raw_sql_here = "alter table EnvironmentalAnswer drop column `".$sid."`";
					$query = DB::query(NULL, $raw_sql_here);
					$result = $query->execute();
					$personQ->delete();
				}
